...
*/
    if(q==NULL)// skontrolujeme ci je q validne
    {
        queueError(QERR_INIT);
    }
    else// uz spravny parameter
    {
        for(int i=0; i<QUEUE_SIZE; i++) //iteruje a plni pole znakom '*'
        {
            q->arr[i]='*';
        }
        q->f_index=0;// nastavime index na 0
        q->b_index=0;// nastavime index na 0
    }
...
*/
    return (index+1)%QUEUE_SIZE;// posunie sa o jedno,v pripade ze je na konci tak prejde na zaciatok
}
...
*/
    return (q->f_index == q->b_index);// prvy prvok sa rovna poslednemu
}
...
*/
    return(nextIndex(q->b_index) == q->f_index);// skontrolujeme ci
}
...
*/
    if(queueEmpty(q)==0)// ak je fronta neprazdna tak
    {
        *c=q->arr[q->f_index];// tlozime znak ktory je na zaciatku fronty
    }
...
*/
    ...
    else// v pripade ak je fronta prazdna
    {
...
*/
    if(queueEmpty(q)==0)// ak je fronta neprazdna tak
    {
        queueFront(q, c);// vratime znak
        queueRemove(q);// odstranime
    }
    else// v pripade ak je fronta prazdna
    {
        queueError(QERR_GET);
...
*/
    if(queueFull(q)==0)// ak je fronta neprazdna tak
    {
        q->arr[q->b_index] = c;// vlozime znak
        q->b_index = nextIndex(q->b_index);// posunieme sa
    }
    else// v pripade ak je fronta plna vypiseme chybu
    {
        queueError(QERR_UP);
    }
}
