...
**/
    if (*RootPtr == NULL)
    {
        *RootPtr = malloc(sizeof(struct tBTNode));
        ...
        if (RootPtr == NULL) //alokacia failed
        {
            return;
        }
    ...
    } else
    {
        ...
        tBTNodePtr NewN = NULL;

        while (NewN == NULL) {
            if (Content < item->Cont)//hodnota je menšia ideme do laveho podstromu
            {
                if (item->LPtr != NULL)//Ked sa da ideme do laveho podstromu
                {
                    item = item->LPtr;
                } else
                {
                    NewN = malloc(sizeof(struct tBTNode));

                    if (NewN == NULL) //alokacia failed
                    {
                        return;
                    }
            ...
            } else if (Content > item->Cont) //hodnota je vacsia ideme do prava
            {
                if (item->RPtr != NULL) // ideme do praveho podstr. ked sa da
                {
                    item = item->RPtr;
                } else
                {
...
**/
    while (ptr != NULL)
    {
        ...
        BTWorkOut(ptr);
        ptr = ptr->LPtr; //dalsi uzol dolava
    }
}
...
**/
    if(!RootPtr)
         return;
    tStackP stack; // inicializacia zasobnika
    SInitP(&stack);
   
    Leftmost_Preorder(RootPtr, &stack);
    tBTNodePtr item_PRE;
    while(!SEmptyP(&stack)) //kym nebude prazdny zasobnik
    {
          item_PRE = STopPopP(&stack);
          Leftmost_Preorder(item_PRE->RPtr, &stack);
    }
}
...
**/
    if(!RootPtr)
        return;
    tStackP stack; //inicializacia zasobniku
    SInitP(&stack);
    Leftmost_Inorder (RootPtr, &stack);
    tBTNodePtr item_IN;
    while(!SEmptyP(&stack))//kym nebude prazdny zasobnik
    {
        item_IN = STopPopP(&stack);
        BTWorkOut(item_IN);
        Leftmost_Inorder (item_IN->RPtr, &stack);
    }
}
/*                                 POSTORDER                                  */
...
**/
    while (ptr != NULL)
    {
        SPushP(StackP, ptr); //ulozenie uzla na zasobnik
        SPushB(StackB, true); //ulozenie informacie
        ptr = ptr->LPtr; //dalsi uzol dolava
    }
}
...
**/
    ...
    SInitP(&stack);
    SInitB(&bstack);
    tBTNodePtr item_POST  = RootPtr;
    Leftmost_Postorder (item_POST, &stack, &bstack);

    while(!SEmptyP(&stack))
    {
        item_POST = STopPopP(&stack);
        SPushP(&stack,item_POST);
        pom_v = STopPopB(&bstack);
        if(pom_v)
        {
            SPushB(&bstack,FALSE);
            Leftmost_Postorder(item_POST->RPtr, &stack, &bstack);
        }
        else
        {
            STopPopP(&stack);
...
**/
    ...
    while (*RootPtr || !SEmptyP(&stack)) //kym ukazatel na rootptr neni NULL
    {
        if (!*RootPtr)
        {
            *RootPtr = STopPopP(&stack);
        }
        if ((*RootPtr)->RPtr) // ked ma koren pravy pods. ulozime na zasobnik
        {
            SPushP(&stack, (*RootPtr)->RPtr);
        }

        node_DEL = *RootPtr; //del korena a novy bude lavy podstrom
